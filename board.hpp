  ...
  public:
    ...
    void DropBlocks(int line, WINDOW *window) {
		for(int col=line;col>1;col--){//Starting at the cleared line
			for(int block=0;block<=width;block++){//Look at each block in the line
				if(col+1 < height && blocks[col][block] && !blocks[col+1][block]){//if we can check the next column and the current block is filled but the block one column down isn't
					int detachedCol = col;
					while(detachedCol+1 < height && blocks[detachedCol][block] && !blocks[detachedCol+1][block]){
