...
class Tetromino {
public:
    Tetromino(const vector<vector<bool>>& shape) {
        this->shape = shape;
    }
...
void GameLoop() {
	...
	string currTetrominoLocation = "Current X: "+to_string(tetromino.x) + " Y: "+to_string(tetromino.y) + "";
	mvwprintw(nextblock, 1, 5, currTetrominoLocation.c_str());
	mvwprintw(nextblock, 2, 5, ("Tet: "+to_string(board.IsBlockFilled(tetromino.x,tetromino.y))).c_str());
	mvwprintw(nextblock, 3, 5, ("Colliding: "+to_string(tetromino.IsCollidingWith(board))).c_str());
	touchwin(nextblock);
	wrefresh(nextblock);
	box(highscore, 0,0);
	...
	wrefresh(highscore);
        ...
        switch (ch) {
            case 'h':
                ...
                break;
            case 'l':
				if(tetromino.y<windowX){
					tetromino.MoveRight();
				}
                break;
            case 'j':
				if(tetromino.x<windowY){
					tetromino.MoveDown();
				}
                break;
            case 'k':
