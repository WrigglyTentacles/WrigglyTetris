...
public:
  ...
  void SetBlock(int x, int y, bool filled) { 
		this->blocks[y][x] = filled;
	}
...
public:
  ...
  Shape(char predefinedShape) {
    ...
    case 'L':
      ...
      this->blocks_used = {{0, 1}, {0, 2}, {-1, 0}, {-2, 0}, {-3, 0}, pivot};
      break;
    }
  };
...
void GameLoop() {
  ...
  //Tetromino tetromino('L', 4, windowY / 2);
  Tetromino tetromino('L', 32, windowY / 2);
  noecho();
  curs_set(0);
  refresh();
  ...
  while (true) {
    ...
    mvwprintw(highscore, 1, 18, ("" + to_string(board.highScore)).c_str());
    touchwin(highscore);
    wrefresh(highscore);
    // ###################################-Screen-###############################
    ...
    case 'h':
      ...
      break;
    case 'l':
      if (tetromino.GetRightBoundary() < windowY - 1 &&
          !board.IsBlockFilled(tetrominox, tetromino.GetRightBoundary() + 1)) {
        tetromino.MoveRight();
      }
      break;
    case 'j':
